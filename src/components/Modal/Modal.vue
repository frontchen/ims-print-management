<template>
  ...
  >
    <div>
      <slot>
        <el-form
          ...
          ref="form"
        >
          <el-row v-for="(row, index) in modalList" :key="index">
            <el-col :class="rowCls(item)" v-for="(item, i) in row" :key="i" :span="colSpan">
              <el-form-item :label="item.label" :prop="item.name">
                ...
                >
                  <span v-if="item.attr.prefix" slot="prefix">{{item.attr.prefix}}</span>
                  <span v-if="item.attr.suffix" slot="suffix">{{item.attr.suffix}}</span>
                  <span
                ...
                <span
                  class="textarea-num"
                  v-if="item.attr.isNumber"
                >{{`${item.attr.txtVal||0}/${item.attr.maxlength}`}}</span>
                <!-- select -->
                <el-select
                  :style="conWidthStyles(item.width)"
                ...
                <el-cascader
                  ...
                  :clearable="item.attr.clearable"
                  :placeholder="item.attr.placeholder"
                  :show-all-levels="item.attr.changeOnSelect"
                ...
                ></el-date-picker>
                <!-- text -->
                <div :style="conWidthStyles(item.width)" class="text" v-if="item.type==='text'">
                  <span v-if="item.value">{{item.value}}</span>
                  <span v-else class="text-placeholder">{{item.attr.placeholder}}</span>
                </div>
              </el-form-item>
            </el-col>
          </el-row>
    ...
    <div slot="footer">
      <slot name="footer">
        <el-button type="text" size="large" @click="modalCancel">取消</el-button>
        <el-button
          type="primary"
          size="large"
          :loading="loading"
